...
namespace tuilib {
    enum class TextAttribute {
        ...
        FORE_EXTENDED           = 38, 	//前景拡張 	前景に拡張色の値を適用します (後の詳細を参照)
        FORE_DEFAULT 	        = 39, 	//前景既定値 	既定値の前景部分のみを適用します (0 を参照)
        ...
        BACK_EXTENDED           = 48, 	//背景拡張 	背景に拡張色の値を適用します (後の詳細を参照)
        BACK_DEFAULT            = 49, 	//背景既定値 	既定値の背景部分のみを適用します (0 を参照)
