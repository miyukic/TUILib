...
class TUILib {
    ...
    static constexpr int BAIRITU = 5;
    inline static uint16_t hight = 6 * BAIRITU;
    inline static uint16_t width = 23 * BAIRITU;
    ...
    static void onTimer() {
        ...
        for (auto i = 0; i < hight; ++i) {
            for (auto j = 0; j < width; ++j) {
                scrn.setCharactor(i + 1, j + 1, std::to_string(counter), { tuilib::TextAttribute::BRIGHT_BACKGROUND_GREEN });
            }
        }
        scrn.reflectScreen();
        ++counter;
